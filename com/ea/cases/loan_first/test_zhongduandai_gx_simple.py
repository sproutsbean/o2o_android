...
class MyTestCase(unittest.TestCase):
    ...
    def test_a_loan_apply(self):
        ...
        MyTestCase.loanno, MyTestCase.partner_zx_no = loan.loan_zhengxin_apply(self.driver, self.loan_type, self.cardNo, self.first_name, self.second_name, self.first_english_name, self.last_english_name, self.phoneNo, gl.xd_manager_name,
                                                                               sumofmoney,
                                                                               self.operate_mode,
                                                                               channel_name, self.screenshot_path, casename, sd_customer_name=sd_customer_name, marry_status=self.marry_status)
    ...
    def test_d_caiwu_approve(self):
        ...
        try:
            current_approver, node_name = tools.get_approver_acount_by_yewuno(self.loanno, process_type)
            if not current_approver:
                raise Exception
            web_login.login(self.webdriver, username=current_approver)
            todopage = TodoPage(self.webdriver)
            approvecaiwupage = ApproveCaiwuPage(self.webdriver)
            # 进入待办查询页面
            web_menu.go_to_wait_todo_query(self.webdriver)
            ...
            approvecaiwupage.click_confirm_button()
            time.sleep(3)
            web_menu.go_to_platform_caiwu_approve(self.webdriver)
    ...
    def test_e_edit_inside_apply(self):
        ...
        try:
            ...
            insidepage = WebInsidePage(self.driver)
            web_menu.go_to_inside_approve(self.driver)
    ...
    # def test_f_start_inside_apply(self):
    #     u"""启动内审流程"""
    #     print("启动内部审批用例开始")
    #     print("使用的贷款单号是:", self.loanno)
    #     casename = sys._getframe().f_code.co_name
    ...
    #         time.sleep(5)
    #         insidepage.click_loan_no(self.loanno)
    #         # 借款人基本信息
    ...
    def test_h_pay_money_apply(self):
        u"""新建打款流程"""
        casename = sys._getframe().f_code.co_name
        # self.fullname = "伏兴桌"
    ...
    def test_i_pay_money_approve(self):
        u"""审批打款流程"""
        casename = sys._getframe().f_code.co_name
