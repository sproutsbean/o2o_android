...
from com.ea.pages import menu_page, loan_apply_list_page
import os
import sys
import time
...
class MyTestCase(unittest.TestCase):
    ...
    def test_loan_apply_list_check(self):
        u"""贷款申请查询页面所有选项检查"""
        casename = sys._getframe().f_code.co_name
        try:
            listpage = loan_apply_list_page.LoanApplyListPage(self.driver)
            ...
            # self.assertTrue(tools.have_element(self.driver, "门店名称", 10))
            # self.driver.back()
            ...
            self.assertTrue(tools.have_element(self.driver, "客户姓名", 10))
            self.driver.back()
            ...
            self.assertFalse(tools.have_element(self.driver, "补企业", 3))
            self.driver.back()
            listpage.click_baozhengjin_guanlifei()
            self.assertTrue(tools.have_element(self.driver, "保证金费率",10))
            self.driver.back()
            if status == "已放款":
                listpage.click_huankuan_jihua_xinxi()
                self.assertTrue(tools.have_element(self.driver, "还款期数", 10))
