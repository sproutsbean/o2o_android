...
from com.ea.pages import menu_page, inside_approve_list_page
import os
import sys
import time
...
class MyTestCase(unittest.TestCase):
    ...
    def test_inside_approve_list_check(self):
        u"""内部审批页面所有选项检查"""
        casename = sys._getframe().f_code.co_name
        try:
            listpage = inside_approve_list_page.InsideApproveListPage(self.driver)
            ...
            if marry_status == "已婚":
                ...
                self.driver.back()
            listpage.click_daikuan_xinxi()
            self.assertTrue(tools.have_element(self.driver, "上传内审资料", 10))
            self.driver.back()
            ...
            self.driver.back()
            listpage.click_fuqi_zhengxin_baogao_xinxi()
            self.assertTrue(tools.have_element(self.driver, "征信单号", 10))
            self.driver.back()
