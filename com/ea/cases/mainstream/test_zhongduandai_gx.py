...
class MyTestCase(unittest.TestCase):
    ...
    def test_c_loan_apply(self):
        ...
        try:
            ...
            loanapplypage.input_sd_customer_name(sd_customer_name)
    ...
    def test_e_caiwu_approve(self):
        ...
        driver = tools.get_chrome_driver()
        web_login.login(driver)
        try:
            todopage = TodoPage(driver)
            approvecaiwupage = ApproveCaiwuPage(driver)
            # 进入待办查询页面
            web_menu.go_to_wait_todo_query(driver)
            todopage.input_yewuno(self.loan_no)
            ...
            approvecaiwupage.click_confirm_button()
            time.sleep(3)
            web_menu.go_to_platform_caiwu_approve(driver)
            actual_result = approvecaiwupage.get_result(self.loan_no)
