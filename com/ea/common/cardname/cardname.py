...
def full_name():
    _last = last_name(1, last_names)
    return "{}{}".format(_last, first_name(random.randint(1, 2), _last, first_names))
...
def convert(ch):
    ...
    with open(resource_path + r'/convert-utf-8.txt', encoding='gbk') as f:
        for line in f:
            if ch in line:
...
def get_name():
    ...
    english_name = []
    for n in name:
        english_name.append(convert(n))
    return name, name[0], name[1:], ''.join(english_name)
...
def get_names():
    ...
    last_english_name = []
    for n in name[0]:
        first_english_name.append(convert(n))
    for n in name[1:]:
        last_english_name.append(convert(n))
    full_english_name = ''.join(first_english_name) + ''.join(last_english_name)
    return name, name[0], name[1:], full_english_name, ''.join(first_english_name), ''.join(last_english_name)
